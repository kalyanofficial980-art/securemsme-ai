...
import { RealModulesPanel } from "@/components/RealModulesPanel";
...
export default async function RealModulesPage({
  params,
  searchParams,
}: {
  ...
  if (!user) {
    redirect("/login?message=Please login to view real modules");
  }
  ...
  if (!scan) {
    redirect("/dashboard?message=Real module report not found");
  }
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <Link
              href={`/report/${scan.id}/authorized-pentest`}
              className="text-sm font-bold text-slate-600"
            >
              Back to authorized security review
            </Link>
            ...
            <p className="mt-3 text-slate-600">
              Actual HTTP, TLS, DNS, and controlled service evidence for
              verified websites.
            </p>
          </div>
