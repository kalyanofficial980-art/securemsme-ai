    ...
    <main className="min-h-screen bg-[#f6f8fb] text-slate-950">
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-6 py-10 lg:grid-cols-[1fr_340px]">
        <div>
          ...
          <form action={addWebsite} className="mt-6 border border-slate-300 bg-white">
            <div className="border-b border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold">Website details</div>
            <div className="space-y-5 p-5 sm:p-6">
              {message ? <div className="border-l-2 border-red-600 bg-red-50 p-4 text-sm font-medium text-red-900">{message}</div> : null}
              <div>
                ...
                <input id="name" name="name" placeholder="Acme marketing site" className="mt-2 w-full border border-slate-300 bg-white px-3.5 py-3 text-sm outline-none focus:border-blue-700" />
              </div>
              <div>
                ...
                <input id="url" name="url" required inputMode="url" placeholder="https://example.com" className="mt-2 w-full border border-slate-300 bg-white px-3.5 py-3 text-sm outline-none focus:border-blue-700" />
              </div>
              <div className="border-t border-slate-200 pt-5">
                <button className="bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-800">Save website</button>
              </div>
            </div>
          </form>
        </div>
