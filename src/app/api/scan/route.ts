...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (userError || !user) {
      return NextResponse.json({ error: "Please login before scanning a website." }, { status: 401 });
    }
    ...
    if (savedWebsiteId) {
      const { data: savedWebsite } = await supabase
        ...
        .single();
      if (!savedWebsite?.url) {
        return NextResponse.json({ error: "Saved website was not found." }, { status: 404 });
      }
      websiteUrl = savedWebsite.url;
    }
    if (!websiteUrl) {
      return NextResponse.json({ error: "Please enter a website URL or select a saved website." }, { status: 400 });
    }
    ...
    if (!reservationId) {
      return NextResponse.json({ error: "Scan quota could not be reserved safely." }, { status: 500 });
    }
    ...
    const canonicalSummary =
      canonicalRiskLevel === "High"
        ? "Verified security evidence includes a high-impact issue. Fix the highest-priority verified issue before treating the public posture as low risk."
    ...
    const baseReport = {
      ...
      diagnosticScores: {
        ...
        note: "Diagnostic and supplemental scores do not replace the canonical evidence-backed Security Score.",
      },
    };
    ...
    const fullReport = { ...baseReport, advancedAudit, retestComparison };
    ...
    try {
      ...
      return NextResponse.json({ error: "Scan completed but could not save report." }, { status: 500 });
    }
    ...
    return NextResponse.json(
      { error: toSafeScanErrorMessage(error, "Scan could not be completed safely. Please check the website URL and try again.") },
      { status: 500 },
    );
  } finally {
