import { runRealTemplateWorkerForScan } from "@/app/report/[id]/real-template-worker/actions";
...
export function RealTemplateWorkerPanel({
  scanId,
  targetUrl,
  verifiedScope,
  savedRuns,
  ...
  return (
    <section className="space-y-8">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-8">
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-black">
              Real path checks with safe evidence storage
            </h2>
            <p className="mt-3 max-w-3xl leading-7 text-slate-600">
              Runs real safe templates against verified website scope using
              GET/HEAD only. Sensitive-path bodies are not stored.
            </p>
          </div>
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Checks security.txt, robots, sitemap, admin/login surfaces, API
            docs, and sensitive debug/config signals with safe storage controls.
          </p>
        </div>

        <form
          action={runRealTemplateWorkerForScan}
        ...
        >
            ...
            {(["light", "standard", "deep"] as PentestIntensity[]).map(
              (intensity) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    {intensity === "light"
                      ? "Trust files and main page checks."
                      : intensity === "deep"
                        ? "Adds sensitive HEAD-only path checks."
                        : "Adds login/admin/API surface checks."}
                  </p>
                </label>
              ),
            )}
          ...
          <label className="mt-6 flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold leading-6 text-amber-950">
            ...
            <span>
              ...
              SecureMSME AI to run safe GET/HEAD template checks only within
              verified scope.
            </span>
          </label>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="mt-2 text-4xl font-black">
            {String(latestSummary.totalTemplates || 0)}
          </p>
        </div>
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          <p className="text-sm font-bold text-emerald-700">Executed</p>
          <p className="mt-2 text-4xl font-black text-emerald-950">
            {String(latestSummary.executedTemplates || 0)}
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5">
          <p className="text-sm font-bold text-blue-700">Observations</p>
          <p className="mt-2 text-4xl font-black text-blue-950">
            {String(latestSummary.observations || observations.length)}
          </p>
        </div>
      </div>
        ...
        <div className="mt-6 grid gap-5">
          {findings.length ? (
            findings.map((finding) => (
              <div
                key={`${finding.templateId}-${finding.customerName}`}
            ...
            <p className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-slate-600">
              No template findings saved yet.
            </p>
          )}
        </div>
      </div>
              ...
              >
                ...
                {observation.blockedBodyStorage ? (
                  <p className="mt-2 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">
                    Sensitive-path body was not stored.
                  </p>
                ) : null}
              </div>
            ))
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-8">
        <h3 className="text-2xl font-black">Saved template runs</h3>
        <div className="mt-6 grid gap-4">
          {savedRuns.length ? (
            savedRuns.map((run) => (
              <div
            ...
            <p className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-slate-600">
              No real safe template runs yet.
            </p>
          )}
        </div>
      </div>
