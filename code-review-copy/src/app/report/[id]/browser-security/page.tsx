...
import { BrowserSecurityPanel } from "@/components/BrowserSecurityPanel";
import { Navbar } from "@/components/Navbar";
import { createClient } from "@/lib/supabase/server";

export default async function BrowserSecurityPage({
  params,
  searchParams,
}: {
  ...
  if (!user) redirect("/login?message=Please login to view browser security");
  ...
  if (!scan) redirect("/dashboard?message=Browser security report not found");
  ...
  const { data: inventories } = await supabase
    ...
    .select(
      "id, target_url, analyzer_status, summary, browser_security_score, page_count, finding_count, csp_finding_count, cors_finding_count, cookie_finding_count, clickjacking_finding_count, mixed_content_count, external_script_count, high_risk_count, created_at",
    )
  ...
  const { data: findings } = latestInventoryId
    ? await supabase
        .from("browser_security_findings")
        .select(
          "id, category, title, severity, confidence, affected_url, observed_value, expected_value, evidence_summary, business_impact, developer_fix, safe_claim, blocked_claim",
        )
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <Link
              href={`/report/${scan.id}/api-security`}
              className="text-sm font-bold text-slate-600"
            >
              Back to API security scanner
            </Link>
            ...
            <p className="mt-3 text-slate-600">
              CSP, CORS, cookie/session flags, clickjacking, HSTS and
              client-side security signal analysis.
            </p>
          </div>
